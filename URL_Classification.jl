...
function main()
    ...
    # Doesn't work through mapping for some reason
    for key in parts_of_url
        for i=1:length(dict[key])
            dict_hashed[key][i] = transform_to_trig(dict[key][i])
        end
    end
end
...
function transform_to_trig(x::Vector{String})
    ...
    for (ind, val) in enumerate(x)
        map!(trig->trig, trig_vec[ind], hash.([val[i:i+2] for i=1:length(val)-2]))
    end
    return trig_vec
end
...
function map_to_vec(x::Vector{UInt64})
    map_vec = Vector{UInt64}(undef,2053)
    for numb in x
        if numb==0
            continue
        end
